...
__global__ void TriLinearSliceForward(const int nthreads, 
    ...
    for (int elt = 0; elt < batch_size; ++elt) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "trilinear_cuda_forward", ([&] {
                ...
                TriLinearSliceForward<<<GET_BLOCKS(num_kernels),
                                              THREADS_PER_BLOCK, 0,
...
template <typename scalar_t>
__launch_bounds__(THREADS_PER_BLOCK)
__global__ void TriLinearSliceBackward(const int nthreads,
		...
		for(int i=0;i<grid_per_ch;++i)
		{
			...
			atomicAdd(grid_grad + id100_r + shift * (i + grid_per_ch * 0), grad_o_r * w100_r);
			atomicAdd(grid_grad + id010_r + shift * (i + grid_per_ch * 0), grad_o_r * w010_r);
			atomicAdd(grid_grad + id110_r + shift * (i + grid_per_ch * 0), grad_o_r * w110_r);
			atomicAdd(grid_grad + id001_r + shift * (i + grid_per_ch * 0), grad_o_r * w001_r);
			atomicAdd(grid_grad + id101_r + shift * (i + grid_per_ch * 0), grad_o_r * w101_r);
			atomicAdd(grid_grad + id011_r + shift * (i + grid_per_ch * 0), grad_o_r * w011_r);
			...
			atomicAdd(grid_grad + id100_g + shift * (i + grid_per_ch * 1), grad_o_g * w100_g);
			atomicAdd(grid_grad + id010_g + shift * (i + grid_per_ch * 1), grad_o_g * w010_g);
			atomicAdd(grid_grad + id110_g + shift * (i + grid_per_ch * 1), grad_o_g * w110_g);
			atomicAdd(grid_grad + id001_g + shift * (i + grid_per_ch * 1), grad_o_g * w001_g);
			atomicAdd(grid_grad + id101_g + shift * (i + grid_per_ch * 1), grad_o_g * w101_g);
			atomicAdd(grid_grad + id011_g + shift * (i + grid_per_ch * 1), grad_o_g * w011_g);
			...
			atomicAdd(grid_grad + id100_b + shift * (i + grid_per_ch * 2), grad_o_b * w100_b);
			atomicAdd(grid_grad + id010_b + shift * (i + grid_per_ch * 2), grad_o_b * w010_b);
			atomicAdd(grid_grad + id110_b + shift * (i + grid_per_ch * 2), grad_o_b * w110_b);
			atomicAdd(grid_grad + id001_b + shift * (i + grid_per_ch * 2), grad_o_b * w001_b);
			atomicAdd(grid_grad + id101_b + shift * (i + grid_per_ch * 2), grad_o_b * w101_b);
			atomicAdd(grid_grad + id011_b + shift * (i + grid_per_ch * 2), grad_o_b * w011_b);
...
void TriLinearSliceBackwardLaucher(const torch::Tensor &grad_output, 
    ...
    for (int elt = 0; elt < batch_size; ++elt) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "trilinear_cuda_backward", ([&] {
                ...
                TriLinearSliceBackward<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
                                                    at::cuda::getCurrentCUDAStream()>>>(
